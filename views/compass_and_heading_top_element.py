...
class CompassAndHeadingTopElement(AhrsElement):
    ...
    def __get_heading_box_points__(self, text_vertical_position: int) -> list:
    ...
    def __generate_heading_strip__(self, heading: int):
        things_to_render = []
        for heading_strip in self.__heading_strip_offset__:
            ...
            if (displayed_left == 0) or ((displayed_left % 90) == 0):
                line_x_left = (
                    self.__center_x__ - self.__heading_strip_offset__[heading_strip]
                )
    ...
    def __render_heading_mark__(self, framebuffer, x_pos: int, heading: int):
        ...
        self.__render_heading_text__(
            framebuffer, heading, x_pos, self.__get_heading_text_y_position__()
        )
    ...
    def render(self, framebuffer, orientation: AhrsData):
        ...
        with TaskProfiler(
            "views.compass_and_heading_top_element.CompassAndHeadingTopElement.setup"
        ):
        ...
        with TaskProfiler(
            "views.compass_and_heading_top_element.CompassAndHeadingTopElement.render"
        ):
            ...
            self.__render_hollow_heading_box__(
                [heading_text, notation_text], framebuffer
            )

    def __render_hollow_heading_box__(self, heading_text: list, framebuffer):
        # pylint:disable=expression-not-assigned
        [
            box_element.render(framebuffer)
            for box_element in self.__heading_box_elements__
        ]
        ...
        self.__render_horizontal_centered_text__(
            ...
            [
                self.__center_x__,
                (self.__compass_box_y_position__ - (self.__font_half_height__ * 0.2)),
            ],
            colors.GREEN,
            colors.BLACK,
            0.8,
        ...
        self.__render_horizontal_centered_text__(
            ...
            [
                self.__center_x__,
                self.__compass_box_y_position__ + (self.__font_half_height__ * 1.2),
            ],
            colors.GREEN,
            colors.BLACK,
            0.5,
    ...
    def __get_hollow_heading_box_elements__(self) -> list:
        heading_text_box_lines = self.__get_heading_box_points__(
            self.__compass_box_y_position__
        )

        return [
            drawing.FilledPolygon(heading_text_box_lines, colors.BLACK, False),
            drawing.HollowPolygon(
                heading_text_box_lines, colors.GREEN, self.__thin_line_width__
            ),
        ]

    def __render_heading_text__(
        self, framebuffer, heading, position_x: int, position_y: int
    ):
        """
