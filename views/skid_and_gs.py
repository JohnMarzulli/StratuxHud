...
class SkidAndGs(AhrsElement):
    ...
    def __init__(
        ...
        self.__text_y_pos__ = (
            (self.__font_half_height__ << 2) + g_y_pos - self.__font_half_height__
        )

        self.__skid_y_center__ = int(self.__height__ * 0.7)
        self.__skid_range__ = self.__width__ >> 4

        self.__skid_ball_radius__ = self.__height__ * 0.03
        self.__skid_centering_line_length__ = int(self.__skid_ball_radius__ * 1.3)

        self.__skid_top_edge__ = (
            self.__skid_y_center__ - self.__skid_centering_line_length__
        )
        self.__skid_bottom_edge__ = (
            self.__skid_y_center__ + self.__skid_centering_line_length__
        )
        self.__skid_left_edge__ = (
            self.__center_x__ - self.__skid_ball_radius__ - self.__skid_range__
        )
    ...
    def __render_g_loading__(self, framebuffer, orientation: AhrsData):
        """
        ...
        g_load_text = (
            "{0:.1f}G".format(orientation.g_load) if is_valid else orientation.g_load
        )
        ...
        self.__render_text_with_stacked_annotations_right_justified__(
            framebuffer, [self.__right_border__, self.__text_y_pos__], text_package
        )

    def __get_skid_position__(self, orientation: AhrsData) -> int:
        if orientation is None:
            return self.__center_x__

        skid_amount = self.__skid_range__ * self.__get_skid_amount__(orientation)

        screen_x = self.__center_x__ + skid_amount
        screen_x = max(screen_x, self.__skid_left_edge__)
        screen_x = min(self.__skid_right_edge__, screen_x)

        return screen_x

    def __render_skid_ball__(self, framebuffer, orientation: AhrsData):
        ...
        drawing.renderer.polygon(
            framebuffer, colors.DARK_GRAY, self.__skid_range_box__, False
        )
