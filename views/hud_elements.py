...
def apply_declination(heading: float) -> int:
    """
...
def get_reticle_size(
    distance: float, min_reticle_size: float = 0.05, max_reticle_size: float = 0.20
) -> float:
    """
    ...
    if distance <= IMPERIAL_SUPERCLOSE:
        ...
        return min_reticle_size + (reticle_range * (1.0 - ratio))


def get_heading_bug_x(heading: float, bearing: float, degrees_per_pixel: float) -> int:
    """
...
def run_hud_element(element_type, use_detail_font: bool = True):
    """
...
def run_hud_elements(element_types: list, use_detail_font: bool = True):
    """
    ...
    simulated_traffic = [
        traffic.SimulatedTraffic(max_distance)
        for max_distance in range(100, 100000, 5000)
    ]
    ...
    while True:
        for test_data in simulated_traffic:
            ...
            traffic.AdsbTrafficClient.TRAFFIC_MANAGER.handle_traffic_report(
                test_data.icao_address, test_data.to_json()
            )
